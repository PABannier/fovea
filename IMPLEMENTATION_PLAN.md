...
|---|---|---|
| `wgpu` + WASM complexity slows development | High | Phase 0 technical spike before building WSI stack |
| Browser image decode interop with Rust/wgpu is painful | High | Benchmark browser decode vs Rust decode early |
| OpenSlide cannot run in browser | Expected | Keep OpenSlide native in `fovea-pack` |
| Full polygon rendering too slow | High | LOD: density → centroids → simplified → full |
| Binary format becomes hard to evolve | Medium | Version every manifest and chunk header |
| Memory blowups with million-cell slides | High | Strict cache budgets and chunked loading |
| Hover picking too slow | Medium | Fixed-grid chunk index and bbox prefiltering |
| Tile generation storage too large | Medium | WebP/JPEG compression, sparse tile skipping |
| Annotation edits complicate immutable chunks | Medium | Delta layer over base model outputs |
| Safari/WebGPU instability | Medium | Chrome-first MVP; Safari later |
| Debugging WASM performance is hard | High | Built-in metrics overlay from Phase 0 |

---

## 9. Open Questions

These are the questions to answer next to finalize v0.2.

### Product scope

1. Is v1 for **ML debugging**, **pathologist annotation**, or both?
2. Is polygon editing required in v1, or can v1 be view/inspect only?
3. Should the first public artifact be a **library**, a **demo app**, or both?

### Slide ingestion

4. Which WSI formats must be tested first? SVS only, or also NDPI/MRXS/SCN?
5. Is static precomputation acceptable for all v1 use cases?
6. Do we need dynamic tile serving later?

### Overlay data

7. What is the realistic max cell count per slide?
8. What is the average polygon vertex count from the segmentation model?
9. Are polygons always simple, or do they include holes/multipolygons?
10. What input format do model outputs currently use: Parquet, GeoJSON, COCO, AnnData, something else?

### Heatmaps

11. Are heatmaps raster outputs, per-cell scalar values, or spatial transcriptomics-like matrices?
12. Do multiple heatmaps need to be active simultaneously?
13. Do heatmaps need scientific exactness or just visualization fidelity?

### Performance target

14. What is the reference hardware?
15. Is Chrome-only acceptable for MVP?
16. Is 60 FPS mandatory, or is 30 FPS acceptable during heavy overlay rendering?

### Architecture

17. Should TypeScript fetch all resources, or should Rust/WASM initiate fetches?
18. Should the viewer support local files in v1?
19. Should `fovea-pack` emit a folder bundle or a single archive file?
20. Do we want a native desktop viewer later? If yes, the Rust core should be designed for reuse outside WASM.
...
```text
- Native Rust fovea-pack
- OpenSlide ingestion
- Static Fovea bundle
- Rust/WASM WebGPU renderer
- Image tile pyramid rendering
- Cell centroids
- Polygon outlines at high zoom
- One heatmap layer
- Hover/click picking
...
```text
- In-browser OpenSlide
- Collaborative annotation
- Full polygon editing
- GPU picking
- Dynamic tile server
- DICOM WSI
- OME-Zarr spatial omics matrix browser
```

The most important engineering discipline:

> Fovea should be benchmark-driven. Every feature needs a performance budget before it is accepted into the renderer.

That is how we avoid building a beautiful but sluggish pathology viewer.

---

## 11. Immediate Next Steps

### 11.1 Week 1: Architecture Spike

```text
- Create Rust workspace
- Create fovea-viewer crate
- Create fovea-js package
- Initialize WebGPU canvas from Rust/WASM
- Draw triangle, textured quad, and synthetic points
- Add frame-time overlay
```

Success criterion:

```text
Rust/WASM WebGPU rendering works in browser with basic camera pan/zoom.
```

### 11.2 Week 2: Synthetic Overlay Benchmark

```text
- Generate synthetic 100k × 100k slide world
- Generate 1M synthetic cells
- Implement fixed-grid spatial chunks
- Render centroids with LOD
...
```text
50k visible centroids render at 60 FPS with stable memory.
```

### 11.3 Week 3: OpenSlide Packer

```text
- Implement OpenSlideReader abstraction
- Read real SVS metadata
- Generate image tile pyramid
- Write manifest.json
...
```text
A real SVS becomes a static Fovea bundle.
```

### 11.4 Week 4: End-to-End Demo

```text
- Load real slide bundle in browser
- Render image tiles
- Load synthetic or real cell chunks
- Render cells over WSI
- Add hover/click picking
```

Success criterion:

```text
A pathologist or ML engineer can open a slide, pan/zoom, inspect cells, and see performance metrics.
```
...
- [ ] Texture cache
- [ ] Tile scheduler
- [ ] Fallback low-res tiles
- [ ] Performance overlay

### Native packing

- [ ] OpenSlide reader abstraction
- [ ] Metadata extraction
- [ ] Tile pyramid writer
- [ ] Manifest writer
- [ ] Thumbnail writer
...
- [ ] LOD generation
- [ ] Binary chunk writer
- [ ] Chunk parser in WASM
- [ ] Centroid renderer
- [ ] Polygon outline renderer
- [ ] Selection highlight renderer

### Heatmaps

- [ ] Heatmap input parser
- [ ] Heatmap pyramid writer
- [ ] Heatmap manifest
- [ ] Single-channel texture upload
- [ ] WGSL colormap shader
